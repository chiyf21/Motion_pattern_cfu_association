...
python run_motion_to_patterns.py \
  --patch-motion-cache /path/to/SliceXX/01_patch_motion \
  --slice 6 \
  --output-root /path/to/output \
  --through-stage 06
```

This is the collaborator handoff mode and does not require raw motion H5. The
raw-H5 mode is also available with `--raw-motion-dir` plus
`--invalid-mask-npy` when the full registered motion dataset is accessible.
